...
二年级小朋友用的英语课文点读 App：把课文音频和网页点读台一起打包进安卓安装包，**完全离线**、不依赖浏览器，手机竖屏 / 平板横屏自适应。内置**女生（粉紫）/ 男生（蓝色）两套外观**，可在顶栏一键切换。

## 功能特性

- **点读台网页**：按单元折叠的课文列表，点任意一条即播放对应 mp3；保留原始英文文件名（不翻译、不重命名）。
- **儿童向界面 + 双外观**：默认粉紫糖果风（女生）/ 蓝色科技风（男生），顶栏「女生 / 男生」一键切换并记住选择；圆角卡片、内联 SVG 图标（不用 emoji），已听条目显示绿色小星星「集贴纸」。
...
- v1.8（versionCode 9）：新增「男生」蓝色外观，顶栏「女生 / 男生」一键切换，选择本地记忆（首屏无闪烁）。
